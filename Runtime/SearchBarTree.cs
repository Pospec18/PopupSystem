...
{
    internal class SearchBarTree<T>
    {
        ...
        private class SearchNode : ISearchNode
        {
            ...
            private static readonly char[] escapeCharacters = { ':', '-', '.', '/', '_' };
        ...
        {
            string keyLower = key.ToLower();
            if (keyLower.Length > 0 && items.TryGetValue(keyLower[keyLower.Length - 1], out ISearchNode node))
